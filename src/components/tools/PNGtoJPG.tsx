...
export default function PNGtoJPG() {
  ...
  const convertToJPG = useCallback(async (imageData: string): Promise<{ blob: Blob; dataUrl: string }> => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Fill background with selected color (for transparent PNGs)
    ...
    try {
      const updatedImages = await Promise.all(
        images.map(async (image) => {
          ...
          const { blob, dataUrl } = await convertToJPG(image.original);
          return {
            ...image,
            converted: dataUrl,
  ...
  const handleDownload = (image: typeof images[0]) => {
    ...
    link.click();
    toast({ title: `${image.name}.jpg downloaded!` });
  };
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <p className="font-medium mb-1">Drop PNG images here</p>
          ...
          <div className="space-y-4 p-4 bg-muted/50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mb-2">
                Replaces transparent areas in PNG
              </p>
          ...
          {images.length === 0 ? (
            <div className="h-64 flex items-center justify-center border rounded-xl bg-muted/30">
              <div className="text-center text-muted-foreground">
                ...
                <p className="text-sm">Add PNG images to convert them to JPG</p>
              </div>
            </div>
          ) : (
                        ...
                        {image.converted && (
                          <>
                            <div>
                              ...
                              <p className="font-medium text-green-600">{formatBytes(image.convertedSize)}</p>
                            </div>
                            <div>
